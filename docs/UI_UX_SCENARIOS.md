...
점글이 앱은 **보호자용 시각 UI**와 **시각장애인용 음성/점자 UX**를 동시에 제공하는 듀얼 UI/UX 구조를 채택했습니다.

### 핵심 원칙

1. **보호자/교사**: 화면에서 학습 진행 상황, 점자 출력 내용, 그래프/문항을 시각적으로 확인
2. **시각장애인**: 음성 명령과 점자 디바이스로 독립적으로 학습 가능
3. **동기화**: 두 인터페이스가 실시간으로 동기화되어 일관된 경험 제공
